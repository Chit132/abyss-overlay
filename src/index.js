...
const { starColor, nameColor, wsColor, fkdrColor, wlrColor, bblrColor, finalsColor, winsColor, getTag, NWL, swLVL, MCColourNames } = require('./helpers.js');
                ...
                $.ajax({type: 'GET', async: true, url: `https://api.hypixel.net/findGuild?key=${key}&byUuid=${uuid}`, success: (data) => {
                    if (data.success === true && data.guild){
                        $.ajax({type: 'GET', async: true, url: `https://api.hypixel.net/guild?key=${key}&id=${data.guild}`, success: (data) => {
                            if (data.success === true && data.guild){
                                if (data.guild.tag) guild = ` <span style="color: ${MCColourNames[data.guild.tagColor]}">[${data.guild.tag}]</span>`;
                            }
